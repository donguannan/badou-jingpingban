...
基于pytorch的网络编写
实现一个网络完成一个简单nlp任务
六分类任务：判断字母a出现的位置(若有重复则为第一个a出现的位置)，样本长度为，a出现的位置为1～5没有出现则为0
"""
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab,hidden_size):
        ...
        self.rnn_layer = nn.RNN(vector_dim, hidden_size, bias=False, batch_first=True) #RNN层
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            return np.argmax(y_pred, axis=1)                #输出预测结果
...
def predict(model_path, vocab_path, input_strings):
    ...
    with torch.no_grad():  #不计算梯度
        result.append(model.forward(torch.LongTensor(x))) #模型预测
        print(input_strings, " :" ,result) #打印结果



if __name__ == "__main__":
    main()
    test_strings = ["abcde", "baswe", "ccaab", "retag", "qwera", "werty"]
